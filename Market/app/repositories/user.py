...
class BaseUserRepository(BaseRepository, ABC):
    ...
    @abstractmethod
    async def update_name(self, user_id: Any, first_name: str, last_name: str, fathers_name: str) -> None:
        ...
    
    @abstractmethod
    async def update_phone(self, user_id: Any, phone: str) -> None:
        ...
...
class UserPostgreRepository(
    ...
    async def update_name(self, user_id: Any, first_name: str, last_name: str, fathers_name: str) -> None:
        stmt = update(self._model).where(self._model.id == user_id).values(
    ...
    async def update_phone(self, user_id: Any, phone: str) -> None:
        stmt = update(self._model).where(self._model.id == user_id).values(phone=phone)
        await self._session.execute(stmt)
        await self._session.commit()
...
class CachedUserPostgreRepository(
    ...
    async def update_name(self, user_id: Any, first_name: str, last_name: str, fathers_name: str) -> None:
        await super().update_name(user_id, first_name, last_name, fathers_name)
    
    async def update_phone(self, user_id: Any, phone: str) -> None:
        await super().update_phone(user_id, phone)