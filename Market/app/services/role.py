...
# Assume this exists in services/role.py
...
class RoleService:
    ...
    async def update_role(self, role_id: Any, dto: RoleUpdateDTO) -> Role:
        role = await self._repository.get(id=role_id)
        if not role:
            raise RuntimeError("Role not found")
        role.update_role(name=dto.name, description=dto.description)
        await self._uow.commit()
        return role
