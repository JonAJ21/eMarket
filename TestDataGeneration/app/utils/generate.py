...
from utils.schemas import Cart, CartItem, CartItemKafka, Category, PaymentOrderEvent, Product, Review, SocialProviderEnum, UserDeviceTypeEnum
...
class DataGenerator:
    ...
    def generate_roles(self):
        ...
        for name, description in roles:
            ...
            self.pg_cursor.execute(
                "INSERT INTO roles (id, name, description) VALUES (%s, %s, %s) ON CONFLICT DO NOTHING",
    ...
    def generate_seller_info(self, user_id):
        ...
        self.pg_cursor.execute(
            ...
            self.fake.bank(), bik, self.fake.boolean(70),
    ...
    def generate_categories(self, num_main_categories=5, max_subcategories=3):
        main_categories = [
            "Электроника", "Одежда", "Дом и сад", 
            "Книги", "Спорт и отдых", "Красота"
        ]
        
        categories = []
        for main_cat in main_categories[:num_main_categories]:
            # Main category
            main_id = str(uuid.uuid4())
            category = Category(
                id=main_id,
                name=main_cat,
                description=f"Все товары категории {main_cat.lower()}"
            )
            self.mongo_db.categories.insert_one(category.model_dump())
            categories.append(main_id)
            
            # Subcategories
            for _ in range(random.randint(1, max_subcategories)):
                sub_id = str(uuid.uuid4())
                sub_name = f"{self.fake.word().capitalize()} {main_cat}"
                sub_category = Category(
                    id=sub_id,
                    name=sub_name,
                    description=f"{sub_name} - лучший выбор",
                    parent_id=main_id
                )
                self.mongo_db.categories.insert_one(sub_category.model_dump())
                categories.append(sub_id)
    ...
    def generate_kafka_events(self, user_ids, num_events=20):
        ...
        for _ in range(num_events):
            ...
            # Send to Kafka
            self.kafka_producer.send(settings.kafka_topic, event.model_dump())